      ...
      <div class="step-card" :class="{ 'active': phase === 1, 'completed': phase > 1 }">
        ...
        <div class="card-content">
          ...
          <div v-if="profiles.length > 0" class="stats-grid">
            ...
            <div class="stat-card">
              <span class="stat-value">{{ selectedProfilesCount }}</span>
              <span class="stat-label">已选Agent数</span>
            </div>
            <div class="stat-card">
          ...
          <div v-if="profiles.length > 0" class="profiles-preview">
            <div class="preview-header">
              <div class="preview-title-group">
                ...
                <span class="preview-subtitle">勾选后再启动模拟，可动态控制本次参与的 Agent 数量</span>
              </div>
              <div class="selection-actions">
                <button class="selection-btn" type="button" @click="selectAllProfiles">全选</button>
                <button class="selection-btn" type="button" @click="clearSelectedProfiles">清空</button>
              </div>
            </div>
            <div class="selection-summary">
              本次模拟将使用 <span class="summary-highlight">{{ selectedProfilesCount }}</span> / {{ profiles.length }} 个 Agent
            </div>
            <div class="profiles-list">
              <div 
                ...
                :key="idx" 
                class="profile-card"
                :class="{ selected: isProfileSelected(profile, idx) }"
                @click="selectProfile(profile)"
              >
                <div class="profile-select" @click.stop>
                  <input
                    :id="`profile-check-${idx}`"
                    type="checkbox"
                    :checked="isProfileSelected(profile, idx)"
                    @change="toggleProfileSelection(profile, idx)"
                  >
                  <label :for="`profile-check-${idx}`">参与模拟</label>
                </div>
              ...
              <div v-if="useCustomRounds" class="rounds-content custom" key="custom">
                ...
                <div class="range-wrapper">
                  ...
                  <div class="range-marks">
                    <span>{{ minimumRounds }}</span>
                    <span 
                      v-if="recommendedRounds > minimumRounds && recommendedRounds < autoGeneratedRounds"
                      class="mark-recommend" 
                      :class="{ active: customMaxRounds === recommendedRounds }"
                      @click="customMaxRounds = recommendedRounds"
                      :style="recommendedMarkStyle"
                    >{{ recommendedRounds }} (推荐)</span>
...
const defaultRecommendedRounds = 40
const customMaxRounds = ref(defaultRecommendedRounds)   // 默认推荐40轮
...
const autoGeneratedRounds = computed(() => {
  ...
  return Math.max(Math.floor((totalHours * 60) / minutesPerRound), minimumRounds)
})

const recommendedRounds = computed(() => {
  if (!autoGeneratedRounds.value) {
    return defaultRecommendedRounds
  }
  return Math.min(defaultRecommendedRounds, autoGeneratedRounds.value)
})

const sliderPercent = computed(() => {
  if (!autoGeneratedRounds.value || autoGeneratedRounds.value <= minimumRounds) {
    return '0%'
  }
  return `${((customMaxRounds.value - minimumRounds) / (autoGeneratedRounds.value - minimumRounds)) * 100}%`
})

const recommendedMarkStyle = computed(() => {
  if (!autoGeneratedRounds.value || autoGeneratedRounds.value <= minimumRounds) {
    return { position: 'absolute', left: '0' }
  }
  return {
    position: 'absolute',
    left: `calc(${((recommendedRounds.value - minimumRounds) / (autoGeneratedRounds.value - minimumRounds)) * 100}% - 30px)`
  }
})
...
const getProfileSelectionId = (profile, idx) => {
  const rawId = profile?.user_id
  const parsedId = Number.parseInt(rawId, 10)
  return Number.isInteger(parsedId) ? parsedId : idx
}

const isProfileSelected = (profile, idx) => {
  return selectedProfileIds.value.has(getProfileSelectionId(profile, idx))
}

const selectedProfilesCount = computed(() => selectedProfileIds.value.size)

const syncSelectedProfiles = (list) => {
  const nextIds = list.map((profile, idx) => getProfileSelectionId(profile, idx))
  if (!selectionTouched.value) {
    selectedProfileIds.value = new Set(nextIds)
    return
  }
  const current = selectedProfileIds.value
  selectedProfileIds.value = new Set(nextIds.filter(id => current.has(id)))
}

const toggleProfileSelection = (profile, idx) => {
  selectionTouched.value = true
  const next = new Set(selectedProfileIds.value)
  const profileId = getProfileSelectionId(profile, idx)
...
const selectAllProfiles = () => {
  ...
  selectedProfileIds.value = new Set(profiles.value.map((profile, idx) => getProfileSelectionId(profile, idx)))
}
...
watch(autoGeneratedRounds, (value) => {
  if (!value) {
    return
  }
  if (customMaxRounds.value < minimumRounds || customMaxRounds.value > value) {
    customMaxRounds.value = Math.min(Math.max(recommendedRounds.value, minimumRounds), value)
  }
}, { immediate: true })
...
const handleStartSimulation = () => {
  // 构建传递给父组件的参数
  const params = {
    selectedAgentIds: Array.from(selectedProfileIds.value).sort((a, b) => a - b)
  }

  if (!params.selectedAgentIds.length) {
    addLog('请至少勾选 1 个 Agent 后再开始模拟')
    return
  }
  ...
  addLog(`本次启用 ${params.selectedAgentIds.length} 个Agent参与模拟`)
...
const fetchProfilesRealtime = async () => {
  ...
  try {
    ...
    if (res.success && res.data) {
      ...
      syncSelectedProfiles(profiles.value)
